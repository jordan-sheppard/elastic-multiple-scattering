...
class ElasticPolarFarfieldEvaluator:
    ...
    def _initialize_radial_coeffs(self) -> None:
        ...
        ### F) Evaluate needed change of basis matrix entries at each point
        # I) Change-of-basis angle
        angles_m_to_mbar = (self.Theta_mbar - self.Theta_m)

        # II) Change-of-basis matrix entries (only need store sines and cosines)
        # NOTE: These will have shape (N_{theta}^{mbar}, N_r^{mbar})
        self.cosine_rotation = np.cos(angles_m_to_mbar)
        self.sine_rotation = np.sin(angles_m_to_mbar)
    ...
    def displacement(self, boundary_only: bool = True) -> np.ndarray:
        ...
        if boundary_only:
            ...
            cosine_rotation = self.cosine_rotation[:,0]     # Shape (N_{theta}^{mbar},)
            sine_rotation = self.sine_rotation[:,0]         # Shape (N_{theta}^{mbar},)
        else:
            R_m = self.R_m                                  # Shape (N_{theta}^{mbar}, N_r^{mbar})
            cosine_rotation = self.cosine_rotation          # Shape (N_{theta}^{mbar}, N_r^{mbar})
            sine_rotation = self.sine_rotation              # Shape (N_{theta}^{mbar}, N_r^{mbar})
        
        # Evaluate farfield derivatives needed for displacement
        ...
        # into mbar-local coordinates)
        u_r_m = dr_m_Kp + dtheta_m_Ks/R_m
        u_theta_m = dtheta_m_Kp/R_m - dr_m_Ks

        # Get rotation matrix entries with correct shapes for grid 
        # (e.g., only at boundary vs. on entire grid of interest)
        if boundary_only:
            cosine_rotation = self.cosine_rotation[:,0]     # Shape (N_{theta}^{mbar},)
            sine_rotation = self.sine_rotation[:,0]         # Shape (N_{theta}^{mbar},)
        ...
        # Get (mbar)-local displacement from (m)-local displacement
        # rotated using rotation matrix entries
        u_r_mbar = cosine_rotation * u_r_m - sine_rotation * u_theta_m
        u_theta_mbar = sine_rotation * u_r_m + cosine_rotation * u_theta_m 

        #  Return mbar-local displacement
        #  (mbar-local radial displacement is index 0 along last axis,
        #  mbar-local angular displacement is index 1 along last axis)
