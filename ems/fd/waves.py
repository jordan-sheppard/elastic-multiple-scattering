...
class IncidentPlanePWaveEvaluator:
    ...
    def _initialize_coordinate_transformation(self) -> None:
        ...
        """
        # For representation in m-local polar coordinates
        self.cosine_rotation = np.cos(-self.Theta_m)
        self.sine_rotation = np.sin(-self.Theta_m)
    ...
    def displacement(
        ...
        # Get global polar displacement needed constants 
        if boundary_only:
        ...
        u_inc_x = u_inc_r_m * self.cosine_local - u_inc_theta_m * self.sine_local
        u_inc_y = u_inc_r_m * self.sine_local + u_inc_theta_m * self.cosine_local
